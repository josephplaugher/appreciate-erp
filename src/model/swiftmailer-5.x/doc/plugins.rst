...
* ``Swift_Plugins_Loggers_printLogger``: Prints output to the screen in
...
.. code-block:: php
    ...
    // Or to use the print Logger
    $logger = new Swift_Plugins_Loggers_printLogger();
    $mailer->registerPlugin(new Swift_Plugins_LoggerPlugin($logger));
    ...
    // NOTE: The printLogger dumps in realtime so dump() does nothing for it
    print $logger->dump();
